...
class WasmTransposeProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    this._ready = false;
  ...
  async _init() {
    try {
      ...
      await __wbg_init();
      this._shifter = new TransposeShifter(globalThis.sampleRate || 48000);
      this._ready = true;
    } catch (err) {
      console.error('[WasmTransposeProcessor] WASM init failed:', err);
    }
  }
...
registerProcessor('wasm-transpose-processor', WasmTransposeProcessor);
